...
// Exported by linker.ld — marks the byte just past kernel BSS.
...
// Mark every page that overlaps [start, end) as used.
static void mark_used(uint32_t start, uint32_t end)
{
    uint32_t p0 = start / PAGE_SIZE;
    uint32_t p1 = (end + PAGE_SIZE - 1) / PAGE_SIZE;
    for (uint32_t p = p0; p < p1 && p < total_pages; p++)
        bitmap_set(p);
}

// Mark only complete pages fully inside [start, end) as free.
